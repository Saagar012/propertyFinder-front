...
export class Home1Component implements OnInit {
  ...
  diffMonthlyPayment: number | null = null;
  diffPayment: number | null = null;
  ...
  calculatePayment(): void {
    const loanAmount = this.principal - this.downPayment;
    const monthlyRate = this.annualRate / 100 / 12;
    const numberOfPayments = this.years * 12;

    if (monthlyRate > 0) {
      this.monthlyPayment = (loanAmount * monthlyRate) / (1 - Math.pow(1 + monthlyRate, -numberOfPayments));
      this.totalPayment = this.monthlyPayment * numberOfPayments;

      const diffLoanAmount = this.principal;
      this.diffMonthlyPayment = (diffLoanAmount * monthlyRate) / (1 - Math.pow(1 + monthlyRate, -numberOfPayments));
      this.diffPayment = this.diffMonthlyPayment * numberOfPayments;
    } else {
      this.monthlyPayment = loanAmount / numberOfPayments;
      this.totalPayment = this.monthlyPayment * numberOfPayments;

      this.diffMonthlyPayment = this.principal / numberOfPayments;
      this.diffPayment = this.diffMonthlyPayment * numberOfPayments;
    }
  }
